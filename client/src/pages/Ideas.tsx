...
import { ArrowRight, Filter, Lightbulb, Search, SlidersHorizontal } from "lucide-react";
...
function competitionClass(level: string) {
  return level === "Low" ? "text-emerald-700 bg-emerald-50" : level === "Medium" ? "text-amber-700 bg-amber-50" : "text-rose-700 bg-rose-50";
}
...
export default function Ideas() {
  ...
  return <DashboardLayout><div className="mx-auto max-w-[1500px] space-y-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-semibold text-violet-600">Opportunity library</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">200 ideas, one focused wedge at a time.</h1><p className="mt-2 max-w-2xl text-muted-foreground">Filter the catalog by market shape, monetization, and competition score. Every card links to a full product thesis and competitor map.</p></div><Button onClick={() => setLocation("/scraper")} variant="outline"><Filter className="mr-2 h-4 w-4" />Add competitor data</Button></div>
