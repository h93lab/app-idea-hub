...
export default function Compare() {
  ...
  return <DashboardLayout><div className="mx-auto max-w-[1500px] space-y-6">
    ...
    {ids.length < 2 ? <Card className="border-dashed"><CardContent className="flex flex-col items-center justify-center py-20 text-center"><GitCompareArrows className="h-10 w-10 text-violet-500" /><p className="mt-4 font-medium">Add at least two idea IDs</p><p className="mt-1 text-sm text-muted-foreground">For example, enter 1, 2, and 3 above.</p></CardContent></Card> : comparison.isLoading ? <div className="flex items-center justify-center py-20 text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading comparison…</div> : <>
      ...
      <Card className="border-0 shadow-sm ring-1 ring-border/60"><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-500" />Competitor strengths, weaknesses, and differentiation</CardTitle></CardHeader><CardContent className="space-y-6">{comparison.data?.map(idea => <section key={idea.id}><div className="mb-3 flex items-center gap-2"><h2 className="font-semibold">{idea.title}</h2><Badge variant="outline">{idea.competitors.length} competitors</Badge></div><div className="grid gap-3 lg:grid-cols-3">{idea.competitors.map(competitor => <div key={competitor.id} className="rounded-2xl border bg-muted/20 p-4"><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{competitor.name}</p><p className="text-xs text-muted-foreground">{competitor.platform}</p></div><Badge variant={competitor.threatLevel === "High" ? "destructive" : "secondary"}>{competitor.threatLevel}</Badge></div><div className="mt-4 space-y-3 text-sm leading-6"><div><p className="font-medium text-emerald-700">Strengths</p><p className="text-muted-foreground">{competitor.strengths}</p></div><div><p className="font-medium text-rose-700">Weaknesses</p><p className="text-muted-foreground">{competitor.weaknesses}</p></div><div><p className="font-medium text-violet-700">How to differentiate</p><p className="text-muted-foreground">{competitor.differentiation}</p></div></div></div>)}</div></section>)}</CardContent></Card>
    </>}
  </div></DashboardLayout>;
}
